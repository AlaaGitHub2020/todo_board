...
class TaskDescriptionField extends HookWidget {
  const TaskDescriptionField({Key? key}) : super(key: key);
      ...
      listener: (context, state) {
        try {
          textEditingController.text =
              state.task.taskDescription?.getOrCrash() ?? "";
      ...
      builder: (context, state) {
        return TextFormField(
          ...
          onChanged: (value) => context.read<CreateNewTaskFormBloc>().add(
                CreateNewTaskFormEvent.taskDescriptionChanged(value),
              ),
          validator: (_) => context
  ...
  InputDecoration buildInputDecoration(BuildContext context) {
    return InputDecoration(
      labelText: S.of(context).typeATaskDescription,
      counterText: "",
    );
  }
}
